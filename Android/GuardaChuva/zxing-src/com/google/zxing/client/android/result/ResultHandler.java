...
import br.com.pvb.guardachuva.R;
import br.com.pvb.guardachuva.RentActivity;
import br.com.pvb.guardachuva.reader.LastScreenView.Screen;
...
public abstract class ResultHandler {
	...
	final void shareByEmail(String contents) {
		sendEmailFromUri("mailto:", null,
				activity.getString(R.string.msg_share_subject_line), contents);
	}
	...
	// TODO
	final void saveProduct(String query) {

		Model.getInstance().setReaderData(query);

		if (Model.getInstance().getLastScreenView() == Screen.Registry) {

			boolean sucessfull = new ProductUtil().createProduct(query);

			if (sucessfull) {

				Toast.makeText(activity, "Salvo com sucesso!",
						Toast.LENGTH_LONG).show();
				
				activity.finish();

				Intent intent = new Intent(Model.getInstance().getContext(),
		...
		} else if (Model.getInstance().getLastScreenView() == Screen.Rent) {
		...
		} else if (Model.getInstance().getLastScreenView() == Screen.Return) {
